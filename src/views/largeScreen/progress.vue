<template>
  <g class="progress">
    <g v-for="(item, index) in d3Data.dataset.nodes.children" :key="index">
      <line v-if="item.children !== undefined" :x1="item.bottomX + 8" :y1="item.bottomY + 5" :x2="item.children[0].imgX + 50" :y2="item.children[0].imgY + 25" style="stroke:#fff;stroke-width:1"></line>
      <g class="nodes" v-for="(item1, index1) in item.children" :key="index1">
        ...
        <g class="node" width="200" height="200">
          ...
          <text dy="4" dx="-4" :x="item1.spreadX" :y="item1.spreadY" fill="#fff" font-size="13" v-if="item1.isFold">+</text>

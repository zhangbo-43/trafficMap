<template>
  ...
  <g id="histogram">
    ...
    <!-- 柱子加字体总高度270 -->
    <g id="pillar" v-for="item in quantityData" :key="item.id" @click="barClick(item)" style="cursor: pointer">
      <g class="pillar-rect">
        <!-- 柱子宽度30 总高度为150 -->
        <rect
            :width="pillarWidth"
...
export default {
  ...
  methods: {
    barClick(params) {
      console.log(params)
      Bus.$emit("nodeMessage", { type:"bar",id: params.id, endX: params.x + 15, endY: params.y })
    }
  },
  beforeMount() {
